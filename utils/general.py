...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    t = time.time()
    #用于储存输出的结果，最后output:List[tensor.size(M,6)],M代表目标数
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # confidence
        # 用obj置信度进行初步筛选
        x = x[xc[xi]]  # confidence#xi:index=0  xc:(1,18900) Bool  xc[xi]:(18900) Bool x:(18900,85)->(71,85)
        ...
        if i.shape[0] > max_det:  # limit detections #限制一下最大目标书
            i = i[:max_det]
        '''
...
def strip_optimizer(f='fan.pt', s=''):  # from utils.general import *; strip_optimizer()
