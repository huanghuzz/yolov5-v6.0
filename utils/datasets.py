...
#定义了一个类（object），当迭代器每次调用__next__的返回值，__iter__返回迭代器的当前迭代次数
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        self.files = images + videos #图片和视频放在一个List容器
    ...
    def __next__(self):
        ...
        if self.count == self.nf:
            raise StopIteration
        #根据目前迭代次数，获取相应图片或者视频的路径
        path = self.files[self.count]
        #判断当前path，是否是视频，如果是就从视频中读取图片
        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video' #切换为视频模式
            # 读取视频的帧（图片），ret_val:Bool用来判断当前帧是否读取正常，img0为读出来的图片
        ...
        #返回路径，padding后的图（用于输入模型），原始的图（），判断是否为视频的标志（如果是图片则为None）
